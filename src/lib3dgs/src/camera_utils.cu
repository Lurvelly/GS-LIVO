...
torch::Tensor compute_perspective_matrix_from_intrinsics(const Eigen::Matrix3f& K, int width, int height, float near, float far) {
    ...
    perspective_mat << 
        ...
        0,                0,                -1,                   0;
using namespace std;
// cout<<"projection_matrix: "<<perspective_mat<<endl;
        
    Eigen::Matrix4f perspective_mat_transposed = perspective_mat.transpose();
// cout<<"projection_matrix_T: "<<perspective_mat_transposed<<endl;
    // Create torch::Tensor from Eigen::Matrix
    auto PTensor = torch::from_blob(perspective_mat.data(), {4, 4}, torch::kFloat32);
